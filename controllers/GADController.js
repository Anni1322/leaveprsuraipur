...
const verifyLogin = async (req, res) => {
  try {
    ...
    if (userData) {
      ...
      if (passwordMatch) {
        // check admin or not
        if (userData.is_GAD === 0) {
          res.render("login", { message: "Email and Password is incorrect." });
        } else {
          req.session.user_id = userData._id;
          
          // send admin home
          res.redirect("/GAD/home");
        }
      } else {
        res.render("login", { message: "Email and Password is incorrect." });
      }
    ...
    if (findUserDepartment) {
      ...
      const userData = await User.findOne({
        ...
        const Pendingadministrator = await Leave.findOne({
          $and:[
            {status:"Pending On Ragistrar"},
            {d_name: userDepartment}
          ]
        }).count();
        const Rejectedadministrator = await Leave.findOne({
          $and:[
            {status:"Rejected from Ragistrar"},
            {d_name: userDepartment}
          ]
        }).count();
        const Approvedadministrator = await Leave.findOne({
          $and:[
            {status:"Approved from Ragistrar"},
            {d_name: userDepartment}
          ]
        }).count();
      ...
      res.render("dashboard", {
        ...
        Rejectedadministrator:Rejectedadministrator,
        Approvedadministrator:Approvedadministrator,

        // vcPending:Pendingadministrator,
        // vcRejected:Rejectedadministrator,
        // vcApproved:Approvedadministrator
        // 
...
const loadadministrator = async (req, res) => {
  try {
    const userData = await User.findById({ _id: req.session.user_id });
    const PendingCount = await Leave.find({ status: "Pending On GAD" }).countDocuments();
    const approvedCount = await Leave.find({ status: "Approved From GAD" }).countDocuments();
...
const loadvcPending = async (req, res) => {
  try {
    const findUserDepartment = await User.findOne({ _id: req.session.user_id });
    if (findUserDepartment) {
      const userDepartment = findUserDepartment.department; 
...
// administrator loadApproved
const loadvcapproved = async (req, res) => {
  try {
    const findUserDepartment = await User.findOne({ _id: req.session.user_id });
    if (findUserDepartment) {
      const userDepartment = findUserDepartment.department; 
...
// administrator loadRejected
const loadvcrejected = async (req, res) => {
  try {
    const findUserDepartment = await User.findOne({ _id: req.session.user_id });
    if (findUserDepartment) {
      const userDepartment = findUserDepartment.department; 
...
const userexportPdf = async (req, res) => {
  try {
      ...
      const html = fs.readFileSync('../views/GAD/htmltopdf.html','utf-8');
    const options = {
      format:'letter'
    }

      pdf.create(html,options).toBuffer('../views/GAD/invoice.pdf',(err,res)=>{
        if(err){
