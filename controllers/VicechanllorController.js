...
const verifyLogin = async (req, res) => {
  try {
    ...
    if (userData) {
      ...
      if (passwordMatch) {
        // check admin or not
        if (userData.is_vicechancellor === 0) {
...
const loadDashboard = async (req, res) => {
  try {
    ...
    const PendingCount = await Leave.find({status:"Pending On Vice Chancellor"}).count();
    const approvedCount = await Leave.find({status:"Approved from Vice Chancellor"}).count();
    const RejectedCount = await Leave.find({status:"Rejected from Vice Chancellor"}).count();
...
const loadpending = async (req, res) => {
  try {
    ...
    const PendingCount = await Leave.find({status:"Pending On Vice Chancellor"});
...
const loadapproved = async (req, res) => {
  try {
    const approvedCount = await Leave.find({status:"Approved from Vice Chancellor"});
    const leave = await Leave.find().exec();
    res.render("approved", {
      title: "approved",
      leaves: leave,
...
const loadrejected = async (req, res) => {
  try {
    const RejectedCount = await Leave.find({status:"Rejected from Vice Chancellor"});
    const leave = await Leave.find().exec();
    res.render("rejected", {
      title: "rejected",
      leaves: leave,
